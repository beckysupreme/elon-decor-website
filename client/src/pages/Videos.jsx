  ...
  const fetchVideos = async () => {
    try {
      ...
      const url = selectedCategory === 'all' 
        ...
        : `http://localhost:5000/api/videos?category=${selectedCategory}`;
        ...
        <section className="bg-gradient-to-r from-[--color-black-bg] to-[--color-dark-gray] py-20">
          <div className="container mx-auto px-4 text-center">
            ...
            <p className="text-gray-300 max-w-2xl mx-auto">
              Watch our decoration setups in action
            </p>
          </div>
        </section>
        ...
        {!videos || videos.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {videos.map((video) => (
              <div 
                key={video.id} 
                className="bg-[--color-dark-gray] rounded-lg overflow-hidden cursor-pointer group hover:transform hover:scale-105 transition-all duration-300"
              ...
              >
                ...
                <div className="p-4">
                  <h3 className="font-semibold text-lg mb-1">{video.title || 'Untitled'}</h3>
                  <p className="text-sm text-gray-400 mb-2">{getCategoryLabel(video.category)}</p>
                  {video.description && (
      ...
      {selectedVideo && (
        ...
        >
          <div className="relative w-full max-w-5xl" onClick={(e) => e.stopPropagation()}>
            <button 
              onClick={() => setSelectedVideo(null)} 
              className="absolute -top-12 right-0 text-white text-3xl hover:text-[--color-gold] transition-colors z-10"
            >
              ×
            </button>
            ...
            <div className="mt-4 text-center">
              <h3 className="text-2xl font-semibold text-white">{selectedVideo.title || 'Untitled'}</h3>
              <p className="text-gray-400 mt-2">{selectedVideo.description || 'No description available'}</p>
              <p className="text-sm text-gray-500 mt-2">
                Category: {getCategoryLabel(selectedVideo.category)}
              </p>
            </div>
          </div>
        </div>
